...
import { getCenteredPlacement, type CenteredPlacement } from "../src/assets/centering";
...
async function collectPngFiles(directories: string[]): Promise<string[]> {
  const files: string[] = [];
...
async function recenterImage(filePath: string, shouldWrite: boolean): Promise<CenteredImageResult> {
...
function formatResult(result: CenteredImageResult, root: string, shouldWrite: boolean): string {
