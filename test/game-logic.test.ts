...
import { createMetaProgress } from "../src/game/meta-progress";
import {
...
import { ENDLESS_STAGE_NUMBER } from "../src/game/stages";
import type {
  BattleState,
  Enemy,
  EnemySpeciesId,
...
const logicSource = readFileSync(new URL("../src/game/logic.ts", import.meta.url), "utf8");

function advance(state: BattleState, ticks: number): BattleState {
  let next = state;
...
function createTower(type: TowerType, x: number, y: number, level = 1): Tower {
  return {
    cooldown: 0,
    id: 1,
    level,
...
function createEnemy({
  ...
  const definition = species === "boss" ? ENEMY_SPECIES.grunt : ENEMY_SPECIES[species];
  return {
...
test("reduceCooldown derives discrete reductions from speed bonus values instead of raw speed levels", () => {
  assert.match(
    logicSource,
    /function reduceCooldown\(cooldown(?::[^,]+)?,\s*bonus(?::[^,]+)?,\s*bonusStep/,
  );
