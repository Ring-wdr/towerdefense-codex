...
import type { ThemeScreenData } from "../screen-data";
...
interface ThemeScreenProps {
  ...
  onEnterBattle: FooterAction["onClick"];
}
...
function getBattleStatusCopy(stageNumber: number, locked: boolean, session: GameSession): string {
  if (locked) {
    return "Entry Locked";
  }
...
function getRouteLabel(index: number, total: number): string {
  return `Route ${index + 1} / ${total}`;
}

function getNodeSlotClassName(index: number): string {
