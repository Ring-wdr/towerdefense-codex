...
export type MetaShopCategory = "global" | "tower" | "combat";
export type MetaShopProgressBucket = "upgrades" | "combatUnlocks";
export type MetaShopEntryId = MetaUpgradeId | CombatUnlockId;

export interface MetaShopLevel {
  price: number;
  value: number;
  unlockStage: number;
}

interface BaseMetaShopEntry {
  id: MetaShopEntryId;
  category: MetaShopCategory;
  label: string;
  description: string;
  maxLevel: number;
  levels: MetaShopLevel[];
}

export interface UpgradeMetaShopEntry extends BaseMetaShopEntry {
  id: MetaUpgradeId;
  progressBucket?: undefined;
}

export interface CombatMetaShopEntry extends BaseMetaShopEntry {
  id: CombatUnlockId;
  progressBucket: "combatUnlocks";
}

export type MetaShopEntry = UpgradeMetaShopEntry | CombatMetaShopEntry;

export const META_SHOP_CATALOG: MetaShopEntry[] = [
  {
    id: "globalStartGold",
    category: "global",
    label: "보급 확장",
  ...
  },
  {
    ...
    maxLevel: 1,
    levels: [{ price: 120, value: 1, unlockStage: 3 }],
  },
  {
    id: "chainSurgeUnlock",
    category: "combat",
    ...
    maxLevel: 1,
    levels: [{ price: 130, value: 1, unlockStage: 5 }],
  },
  {
    id: "deepFreezeUnlock",
    category: "combat",
    ...
    maxLevel: 1,
    levels: [{ price: 140, value: 1, unlockStage: 6 }],
  },
];

const META_SHOP_CATALOG_BY_ID = new Map<MetaShopEntryId, MetaShopEntry>(
  META_SHOP_CATALOG.map((upgrade) => [upgrade.id, upgrade]),
);

function clampUpgradeLevelForIndex(value: unknown, maxLevel: number): number {
...
function getProgressBucket(definition: MetaShopEntry | undefined): MetaShopProgressBucket {
...
function getCurrentLevel(progress: MetaProgress, definition: MetaShopEntry): number {
  const rawLevel = definition.progressBucket === "combatUnlocks"
    ? progress.combatUnlocks[definition.id]
    : progress.upgrades[definition.id];

  return clampUpgradeLevelForIndex(rawLevel, definition.maxLevel);
}

export function getMetaRewardForStage(stageNumber: number): number {
  return 20 + stageNumber * 15;
}

export function awardStageClearRewards(progress: unknown, stageNumber: number): MetaProgress {
...
export function canPurchaseUpgrade(progress: unknown, upgradeId: MetaShopEntryId): boolean {
  const normalized = normalizeMetaProgress(progress);
  ...
  const currentLevel = getCurrentLevel(normalized, definition);
...
export function purchaseUpgrade(progress: MetaProgress, upgradeId: MetaShopEntryId): MetaProgress {
  ...
  if (definition.progressBucket === "combatUnlocks") {
    return {
      ...
      combatUnlocks: {
        ...normalized.combatUnlocks,
        [definition.id]: Math.min(currentLevel + 1, definition.maxLevel),
      },
    };
  }
