...
export type CampaignTheme = "기초 방어" | "압박 대응" | "후반 운용";
export type ThemeName = CampaignTheme | "무한 모드";
export type WaveProfileId = keyof typeof WAVE_PROFILES | "endless-crucible";
export type WaveSpecies = "grunt" | "runner" | "shellback" | "wisp" | "swarmling" | "boss";
...
export interface WaveDefinition {
  ...
  reward: number;
  speed: number;
  spawnPlan: WaveSpecies[];
  speciesPool: WaveSpecies[];
}
...
const GEOMETRY_CACHE = new Map<number, StageGeometry>();

export function getStageCount(): number {
  return STAGES.length;
}

export function getEndlessStageDefinition(): StageDefinition {
  return ENDLESS_STAGE;
}

export function getStageDefinition(stageNumber: number): StageDefinition {
  if (stageNumber === ENDLESS_STAGE_NUMBER) {
    return ENDLESS_STAGE;
  }

  const safeStageNumber = Number.isFinite(stageNumber) ? Math.trunc(stageNumber) : 1;
  return STAGES[clamp(safeStageNumber, 1, STAGES.length) - 1]!;
}

export function getStageWaveDefinition(stageNumber: number, waveNumber: number): WaveDefinition {
  if (stageNumber === ENDLESS_STAGE_NUMBER) {
  ...
  const cycle = WAVE_PROFILES[stage.waveProfile as keyof typeof WAVE_PROFILES][waveNumber - 1]!;
  const stageTier = stage.number - 1;
  const countBonus = Math.floor(stageTier / 2) * 2;
  const spawnPlan = Array.from(
    { length: base.count + countBonus },
...
export function getStagePathCells(stageNumber: number): GridCell[] {
  return getStageGeometry(stageNumber).cells;
}

export function getStagePathLength(stageNumber: number): number {
  return getStageGeometry(stageNumber).totalLength;
}

export function getStagePointAlongPath(stageNumber: number, progress: number): Point {
...
export function isStageRoadCell(stageNumber: number, x: number, y: number): boolean {
  return getStageGeometry(stageNumber).cellSet.has(`${x},${y}`);
}

export function getThemeOrder(): CampaignTheme[] {
  return Array.from(new Set(STAGES.map((stage) => stage.theme as CampaignTheme)));
}
...
function getStageGeometry(stageNumber: number): StageGeometry {
  const stage = getStageDefinition(stageNumber);
...
function getEndlessWaveDefinition(waveNumber: number): WaveDefinition {
  ...
  const profiles: readonly WaveSpecies[][] = [
  ...
  const spawnPlan = Array.from({ length: count }, (_, index) => profile[index % profile.length]!) as WaveSpecies[];
...
function buildPathSegments(points: GridCell[]): Pick<StageGeometry, "segments" | "totalLength"> {
...
function expandPathToCells(points: GridCell[]): Set<string> {
