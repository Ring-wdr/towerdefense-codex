...
function getCommandRow(layout: SceneLayout, count: number, preferredWidth = 180): CommandRowLayout {
...
export function getBattleViewportLayout(
  scene: SceneScaleLike,
  boardWidth: number,
  boardHeight: number,
...
export function getSceneLayout(scene: SceneScaleLike, options: SceneLayoutOptions = {}): SceneLayout {
  const width = scene.scale.width;
  const height = scene.scale.height;
  const safeBottomInset = normalizeInset(options.safeBottomInset);
  ...
  return {
    ...
    getCommandRow(count: number, preferredWidth = 180): CommandRowLayout {
...
export function getViewportFrame(scene: SceneScaleLike, options: SceneLayoutOptions = {}): ViewportFrame {
...
export function getActionLayout(
  frame: ViewportFrame | SceneScaleLike,
  count: number,
  preferredWidth = 180,
): CommandRowLayout {
  const layout = typeof (frame as Partial<ViewportFrame>)?.getCommandRow === "function"
    ? frame as ViewportFrame
    : getViewportFrame(frame as SceneScaleLike);
